...
module.exports = async ({ actions, graphql }, options) => {
  ...
  const showcaseTeaser = data.allKickstartDsShowcasePage.edges.map((page, index) => {
    const teaser = {
      ...
      "body": `${stripHtml(cleanObjectKeys(page.node.showcase).description).result}`,
  ...
  await actions.createPage({
    ...
    context: {
      page: {
        ...
        postTeaser: showcaseTeaser.filter((teaser) => !teaser.categories.some((category) => category.label === 'Internal')),
