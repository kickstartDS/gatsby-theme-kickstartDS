...
const additionalReducer =
  (mod) =>
  (prev, [key, value]) => {
    if (key !== "index" && key.indexOf("/") === -1 && value.length) {
...
const additionalPackage = exportJsons[0].match(
  /node_modules\/(.*)\/dist\/exports\.json/
)[1];
...
const additional = Object.entries({ ...uniquelyNewExports }).reduce(
  additionalReducer(additionalPackage),
  ""
);
